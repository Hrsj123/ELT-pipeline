...
def operations(path, file_name):
    ...
    #### Probably hadoop not setup properly
    # late_carrier_deliveries.coalesce(1) \
    #                        .write \
    #                        .option("header", "true") \
    #                        .csv("../../Output/missed_shipping_limit_orders.csv") # <<<<<<< edit path

    pandas_df = late_carrier_deliveries.toPandas()
    pandas_df.to_csv("./Output/missed_shipping_limit_orders.csv", index=False)
